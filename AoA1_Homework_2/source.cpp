...
int main(int argCount, char** arguments){
    ...
    double updateProb = stod(arguments[2]); //p
    ...
    file.close();
    heap.checkIfOK();   //check if heap is valid
    cout << fixed << setprecision(7);
    for(int i = 0; i < (int)calledTaxis.size(); i++){
        cout << "Distance of the called taxi #" << i+1 << ": " << calledTaxis[i] << "\n";
    }
    cout << setw(10) << right << "Addition: " << addition << "\n";
    cout << setw(10) << right << "Update: "  << update << "\n";

    Clock::time_point end = Clock::now();
    chrono::duration<double> timeElapsed = end - start;
    double timeElapsed_ms = 1000 * timeElapsed.count();
    cout << "Total time elapsed (ms): " << timeElapsed_ms << "\n";
    return 0;
}